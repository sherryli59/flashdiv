...
import numpy as np
from flashdiv.distributions.dw import DW4
...
def parse_args():
    ...
    params.add_argument('--logname', type=str, default='dw')
    params.add_argument('--run', type=int, default=0)
    params.add_argument('--save_every', type=int, default=100)
    params.add_argument('--nparticles', type=int, default=4)
    params.add_argument('--dim', type=int, default=2)
    params.add_argument('--kT', type=float, default=1.0)
    ...
    params.add_argument('--burn_in', type=int, default=20000)
    params.add_argument('--batch_size', type=int, default=1000)
    params.add_argument('--spring_constant', type=float, default=0.0)
    params.add_argument('--adaptive_step_size', action='store_true', default=True)
...
def run_MH(target, x_init, n_steps, dt, adaptive=True, save_every=100, burn_in=1000, xs=None):
    ...
    for i in range(n_steps + burn_in):
        ...
        x_prop = x + dt * torch.randn_like(x)
        # Metropolis ratio
...
if __name__=="__main__":
    ...
    sys = DW4(nparticles=args.nparticles, dim=args.dim, batch_size=args.batch_size,
    ...
    # x_init = x_init + torch.rand_like(x_init)*torch.sqrt(torch.tensor(2*args.kT*0.01))
    x_init = torch.rand((args.batch_size,args.nparticles,args.dim)).to("cuda:0") * boxlength - boxlength/2
