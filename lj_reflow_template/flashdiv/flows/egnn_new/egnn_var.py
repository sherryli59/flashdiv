...
class EGNN_dynamics(FlowNet):
    def __init__(self, n_particles, n_dimension, hidden_nf=64, device='cpu',
        ...
        self.mode = mode
        self._n_particles = n_particles-1  # Exclude the self particle
        self.out_node_nf = out_node_nf if out_node_nf is not None else hidden_nf
        if mode == 'egnn_dynamics':
            self.egnn = EGNN(in_node_nf=1, in_edge_nf=1, hidden_nf=hidden_nf, device=device, 
                             act_fn=act_fn, n_layers=n_layers, recurrent=recurrent, 
                             attention=attention, tanh=tanh, agg=agg, out_node_nf=self.out_node_nf*2+1)
        else:
        ...
        self.self_pot_model = nn.Sequential(
            nn.Linear(self.out_node_nf + 1, hidden_nf),
            act_fn,
            nn.Linear(hidden_nf, 1)
        )

        self.pair_pot_model = nn.Sequential(
            nn.Linear(self.out_node_nf + 2, hidden_nf),
            act_fn,
            nn.Linear(hidden_nf, 1)
        )

        self.com_pot_model = nn.Sequential(
            nn.Linear(1 + 1 + 1, hidden_nf),
            act_fn,
            nn.Linear(hidden_nf, 1)
        )
    ...
    def forward(self, xs, t):
        ...
        if self.mode == 'egnn_dynamics':
            ...
            h_out, x_out = self.egnn(h, x, edges, edge_attr=edge_attr)

        h_out = h_out.view(B, P, P - 1, -1) # Note: h_out[:,i] and x_out[:,i] do not depend on x[:,i]
        self_pot_h = h_out[..., :self.out_node_nf].reshape(-1, self.out_node_nf) # (B * P, out_node_nf)
        pair_pot_h = h_out[..., self.out_node_nf:2*self.out_node_nf].reshape(-1, self.out_node_nf) # (B * P, out_node_nf)
        com_pot_h = h_out[..., 2*self.out_node_nf:].reshape(-1, self._n_particles).sum(dim=-1).unsqueeze(-1) # (B * P, 1)
        diffij = source_xs_com.unsqueeze(2).expand(B, P, P-1, D) - x_out.reshape(B, P, P-1, -1)
        rij = rearrange(
            diffij.norm(dim=-1),'b p p2 -> (b p p2) 1')
        ...
        pot = self.pair_pot_model(
            torch.cat((
                rij,
                t_,
                pair_pot_h), dim=-1)).reshape(B, P, P-1,1)

        # # print(rcom.shape,t.reshape(-1, 1,1).expand(-1, P,  1).reshape(-1,1).shape,  h_final[:,:,:,-1].shape)
        com_pot = self.com_pot_model(
            torch.cat((
                rcom.reshape(-1, 1),
...
class EGNN(nn.Module):
    def __init__(self, in_node_nf, in_edge_nf, hidden_nf, device='cpu', act_fn=nn.SiLU(), n_layers=4, recurrent=True, attention=False, norm_diff=True, out_node_nf=2, tanh=False, coords_range=15, agg='sum'):
