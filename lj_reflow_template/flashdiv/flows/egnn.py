...
class EGNN_dynamics(FlowNet):
    def __init__(self, n_particles, n_dimension, hidden_nf=64, device='cpu',
        ...
        elif mode == "ignn_dynamics":
            self.ignn = IGNN(in_node_nf=1, out_node_nf=n_dimension, in_edge_nf=1, hidden_nf=hidden_nf,  act_fn=act_fn, n_layers=n_layers, recurrent=recurrent, attention=attention, tanh=tanh, agg=agg)
        self.device = device
    ...
    def forward(self, xs, t):
        ...
        elif self.mode == "ignn_dynamics":
            edge_attr = torch.sum((x[edges[0]] - x[edges[1]])**2, dim=1, keepdim=True)
            out = self.ignn(h, x, edges, edge_attr=edge_attr)
            out = out.view(n_batch, self._n_particles, self._n_dimension)
            return out
