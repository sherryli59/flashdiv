...
def run_MALA(target, x_init, n_steps, dt, adaptive=True,
    ...
    for i in range(n_steps + burn_in):
        ...
        # # Plot after burn-in
        # if i == burn_in:
        #     pot_list = torch.stack(pot_list) / 500
        #     plt.plot(pot_list.detach().cpu().numpy())
...
def parse_args():
    ...
    params.add_argument('--periodic', action='store_true', default=True)
