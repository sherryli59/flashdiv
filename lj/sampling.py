...
def run_MH(target, x_init, n_steps, dt, xs=[]):
    '''
    ...
    for t in range(n_steps):
        ...
        x[:,particle_idx,:] += dt * torch.randn_like(x[:,particle_idx,:])
        ratio = target.potential(x_init) - target.potential(x)
        ...
        accs.append(acc)
        if t % 100 == 0:
            xs[t//100] = x.detach().cpu().numpy()
        x_init = x.clone().detach()
    print(target.potential(x))
    return xs, torch.mean(torch.stack(accs).float())

def run_MALA(target, x_init, n_steps, dt, save_every=100,burn_in=1000, xs=[]):
    '''
    target: target with the potentiel we will run the langevin on
    -> needs to have force function implemented
    x (tensor): init points for the chains to update (batch_dim, dim)
...
if __name__=="__main__":
    ...
    x_init = torch.tensor(even_spacing(args.nparticles, args.boxlength, args.dim)).to("cuda:0")
    ...
    traj_list, acc = run_MALA(sys, x_init, args.num_steps, args.step_size, args.save_every, args.burn_in, traj_list)
    #traj_list, acc = run_MH(sys, x_init, args.num_steps, args.step_size, traj_list)
